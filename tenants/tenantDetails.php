...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $tenant_user_id = intval($_GET['tenant_user_id']);
    ...
    $query = "
        ...
        WHERE B.booking_status IN ('Active', 'Notice Given')
          AND B.user_id = ? AND B.booking_id = ?
        GROUP BY B.booking_id
        ORDER BY B.start_date DESC
    ";

    $stmt = $conn->prepare($query);
    $stmt->bind_param("ii" , $tenant_user_id , $booking_id);
    $stmt->execute();
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $row['notice_end_date'] = $notice_end_date;
          ...
          // QR Data (full tenant info)
           ...
           $row['qr_url'] = "https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=" . urlencode($qr_data);
